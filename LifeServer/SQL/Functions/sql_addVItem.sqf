...
_query = ((_query select 0) select 0) select 0;
_query = call compile _query;
_int = 0;
{
    if((_x select 0) == (_ItemObject select 0)) then {
        _added = true;
        _query set [_int, [_ItemObject select 0, format["%1", (_x select 1) + parseNumber(_ItemObject select 1)]]];
    };
    _int = _int + 1;
} forEach _query;
if(!_added) then {
...
//Thinks comas seperate comas but somehow this works on other arrays but not this one?
//Fucking Arma2Net
_update = "Arma2Net.Unmanaged" callExtension format ["Arma2NETMySQLCommand ['serverlife', 'UPDATE playerinfo SET Vitems = '%1' WHERE UID = '%2'']", _query, _uid];
